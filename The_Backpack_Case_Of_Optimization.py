...
#You have the following situation. You have to fill your backpack with the
# products of a Dataframe. You can't get a weight value bigger than 15
# you have to search the solution with the maximum of itens as possible
# and priorize those with more points
...
# sleeping bag           30             7
# rope                   10             5
# dipping compass        30             1

# chromossome has genes which represent the objects
# 1 included
# 0 not included
# sum the points
# restriction: weight < 15
...
class Chromosome:
    ...
    def calculateFitnessValueOfTheChromosome(self, callback):
        return callback(self.genes)


data = {
    "Points": [10, 20, 15, 2, 30, 10, 30],
    "Weight": [1,5,10,1,7,5,1]
}

backpackDf = pd.DataFrame(data, index = ["jackknife", "beans", "potatoes", "flashlight",
"sleeping bag", "rope", "dipping compass"])

print(backpackDf)


def backpackOptimizationFunction(x):
    points = 0
    weight = 0
    for i in range(len(x)):
        if(x[i] != 0):
            point += backpackDf.iloc[i,0]
            weight += backpackDf.iloc[i,1]
    if(weight > 15):
        points = 0
    return points
...
def binaryGeneticAlgorithm(numberOfChromosomes, numberOfGenes, numberOfGenations, crossoverProbability, crossoverType,
    ...
    print(" BEST CHROMOSOME OF THE FIRST GENERATION HAS THIS FITNESS VALUE")
...
binaryGeneticAlgorithm(10,7, 15, 0.90, 'single', 0.10, 0.20,backpackOptimizationFunction)
